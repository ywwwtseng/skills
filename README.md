...
| arch | tech-stack | `/arch:tech-stack` | 多輪問答選型 frontend / mobile / backend / database / infra、架構模式與 **UI 風格**（`tonal-ui` / `calm-ui` / 既有設計系統，兩套視覺語言互斥所以在這裡就寫死），輸出 `docs/architecture/tech-stack.md`；UI 風格同時進「約束與慣例」第一條 |
